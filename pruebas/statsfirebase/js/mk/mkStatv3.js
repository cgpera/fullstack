...
async function getArrayFromURL(urlAddr, options) {
  try {
    const response = await fetch(urlAddr, options).then(res => {
      return  res.json();
    });
    //const json = await ;
    app.miembros = response.results[0].members;
    app.statistics = makeStats(statistics);// Aqui enviamos la estructura de json
  } catch (e) {
